  ...
  angular
    ...
    .constant("dialogConstants", {
      messages: {
        ...
        personalWelcomeMsg:
          "Nice to meet you, %s. What can I do for you? Here are some questions I get asked frequently to get you started:",
        personalReturnWelcomeMsg:
          "Good to see you again, %s! How can I help you today?"
      },
      errorMessages: {
        lowConfError:
          "I'm sorry, I didn't catch that. Could you please try rephrasing your question and include the offering you are inquiring about?",
        bluemixError:
          "Sorry, I'm having trouble connecting to Bluemix. Please come back later!",
        timeoutError:
          "Sorry, I wasn't able to find you an answer. Could you please rephrase your question and include the offering you are inquiring about?",
        serviceError:
          "Oh no, seems like we've hit a Service error! I will log this for troubleshooting. Please try rephrasing your questions."
      },
      url: {
...
(function() {
  ...
  angular
    ...
    .service("dialogService", [
      ...
      "$sce",
      "dialogConstants",
      function(_, $http, $q, $timeout, $sce, dialogConstants) {
        var conversation = [];
        ...
        var getResponse = function(question) {
          if (conversation.length > 1) {
            ...
            return $http
              .post(dialogConstants.url.getWCSResponse, jsonDataResponse)
              .then(
                function(res) {
                  return {
                ...
                .then(
                  function(res) {
                    return {
                      ...
                      context: res.data.context,
                      responses:
                        res.data.output.text && res.data.output.text.length > 0
                          ? res.data.output.text[0]
                          : ""
                    };
                  },
                  function(error) {
        ...
        function truncateText(text) {
          ...
          if (text.length <= 180) {
            ...
            // html tags
            openHTMLTag =
              shortenedString.match(patt1) != null
                ? shortenedString.match(patt1).length
                : null;
            closeHTMLTag =
              shortenedString.match(patt2) != null
                ? shortenedString.match(patt2).length
                : null;
            ...
            if (shortenedString.includes("<a")) {
              ...
              if (!shortenedString.includes("</a>")) {
                var indexLinkCloseTag =
                  text.substring(openATag, text.length).indexOf("</a>") +
                  openATag;
          ...
          return getResponse(input).then(
            function(lastRes) {
              ...
              if (lastRes && lastRes.source !== "ERROR") {
                ...
                if (
                  !lastRes.responses ||
                  lastRes.responses === "undefined" ||
                  (typeof lastRes.responses === "string" &&
                    lastRes.responses.trim().length === 0) ||
                  (typeof lastRes.responses === "string" &&
                    lastRes.responses === "undefined<br>undefined")
                ) {
                  // no response returned from api calls
                  segment.responses =
                    dialogConstants.errorMessages.lowConfError;
                } else {
                  // if returning user, display welcome back message instead of intro greeting message
                  if (
                    conversation.length == 1 &&
                    dialogConstants.customGreeting
                  ) {
                    if (
                      conversation.length == 1 &&
                      newUser === false &&
                      !dialogConstants.skipAuth
                    ) {
                      segment.responses = sprintf(
                        dialogConstants.messages.personalReturnWelcomeMsg,
                        input
                      );
                    } else if (
                      conversation.length == 1 &&
                      ...
                      segment.responses = sprintf(
                        dialogConstants.messages.personalWelcomeMsg,
                        input
                      );
                    } else if (
                      conversation.length == 1 &&
                      ...
                      segment.responses = sprintf(
                        dialogConstants.messages.genericWelcomeMsg,
                        input
                      );
                    }
                  } else {
                    if (lastRes.responses instanceof Array) {
                      if (lastRes.responses.length > 1) {
                        ...
                        segment.responses =
                          typeof lastRes.responses[0] === "string"
                            ? lastRes.responses[0]
                            : lastRes.responses[0].metadata.review_text
                              ? lastRes.responses[0].metadata.review_text
                              : "";
                      }
                    } else {
            ...
            function(error) {
              ...
              if (error.statusText === "error") {
                conversation[index - 1].responses =
                  dialogConstants.errorMessages.bluemixError;
              } else if (error.statusText === "timeout") {
                conversation[index - 1].responses =
                  dialogConstants.errorMessages.timeoutError;
              }
            }
          );
        };
  ...
  var DialogController = function(
    ...
    self.submit = function() {
      ...
      if (self.conversation.length == 0 && newUser == true && valid == false) {
        document.cookie =
          dialogConstants.ibmIdCookie + "=" + self.question.toLowerCase();
        ...
        dialogService.query(self.question).then(
          function(response) {
            $("#question").removeAttr("disabled");
            $("#question").val("");
            $("#question").focus();
            ...
            if (
              $("#scrollable-div").prop("clientHeight") <
              $("#scrollable-div").prop("scrollHeight")
            ) {
    ...
    if (newUser == false) {
      var username = dialogConstants.skipAuth
        ? "guest"
        : getCookie(dialogConstants.usernameCookie);
            ...
            if (
              confirm(
                "Just want to make sure you're " +
                  res[indexEmail].name +
                  ". Please hit OK if that's your name so I can remember you next time we chat! Otherwise, press Cancel and input your IBM ID again."
              ) == true
            ) {
